...
class TestTemporalBatcher:
    ...
    def test_sliding_windows_overlap(self):
        ...
        batches = list(sliding_windows(
            items, duration=timedelta(hours=6), step=timedelta(hours=3),
        ))
    ...
    def test_describe_windows(self):
        from loom.contrib.rag.tools.temporal_batcher import tumbling_windows, describe_windows
        items = self._make_items(12)
        batches = list(tumbling_windows(items, timedelta(hours=6)))
    ...
    def test_window_batch_channel_ids(self):
        from loom.contrib.rag.tools.temporal_batcher import tumbling_windows
        items = [
            self.FakeItem(datetime(2026, 3, 1, tzinfo=timezone.utc), channel_id=1),
            self.FakeItem(datetime(2026, 3, 1, 1, tzinfo=timezone.utc), channel_id=2),
            self.FakeItem(datetime(2026, 3, 1, 2, tzinfo=timezone.utc), channel_id=1),
        ]
...
class TestToolImports:
    def test_all_exports(self):
        from loom.contrib.rag.tools import (
            normalize, NormalizationResult,
            extract_links_from_entities, hazm_normalize,
            WindowBatch, tumbling_windows, sliding_windows,
            daily_windows, describe_windows,
        )
