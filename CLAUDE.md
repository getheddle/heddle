...
Heddle is an actor-based framework for orchestrating multiple LLM agents via NATS messaging. It was built to replace a monolithic AI conversation approach that breaks down as database volume and knowledge graph complexity grow.
...
tests/                    # 82 test files, 1844 unit tests + 1 integration test (90% coverage)
...
- **LLM backends:** `AnthropicBackend` (Claude API, version 2024-10-22), `OllamaBackend` (local models via Ollama), `OpenAICompatibleBackend` (vLLM, llama.cpp, LiteLLM, etc.), and `LMStudioBackend` (an OpenAI-compatible subclass with LM Studio defaults). All support tool-use. `OpenAICompatibleBackend` accepts both `http://host:port` and `http://host:port/v1` base URLs (the trailing `/v1` is normalized away so requests still hit `/v1/chat/completions`). For thinking-style models that emit their answer on `message.reasoning_content` while leaving `content` empty (qwen3.5, deepseek-r1, …), the OpenAI-compatible path falls back to `reasoning_content` and also surfaces it on the response dict / `ChatResponse.reasoning_content` so callers can log or strip it.
