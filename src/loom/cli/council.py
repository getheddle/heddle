...
Commands::
    ...
    loom council run <config> --topic <text>    # Run a council discussion
    loom council validate <config>              # Validate a council config
"""
...
from loom.cli.config import DEFAULT_CONFIG_PATH, resolve_config

if TYPE_CHECKING:
    from loom.contrib.council.schemas import TranscriptEntry
...
def run(
    ...
    click.echo()
    click.echo(click.style("  Loom Council", fg="cyan", bold=True))
...
def validate(config: str) -> None:
    ...
    from loom.contrib.council.config import validate_council_config
