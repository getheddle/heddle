...
Subscribes to ``loom.goals.incoming`` and runs multi-round council
discussions by dispatching :class:`TaskMessage` objects to existing
Loom workers.  Each agent turn becomes a standard worker task, so
...
Pattern follows :class:`loom.orchestrator.pipeline.PipelineOrchestrator`
...
_tracer = get_tracer("loom.council")
...
class CouncilOrchestrator(BaseActor):
    ...
    async def _dispatch_agent_turn(
        ...
        await self.publish("loom.tasks.incoming", msg)
